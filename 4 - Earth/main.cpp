...
int main()
{
    try
    {
        ...
        GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Earth", nullptr, nullptr);
        if(window == nullptr)
        {
        ...
        unsigned int earthTexture = loadTexture("earthTexture.png");
        auto earth = std::make_unique<Earth>(earthTexture);
        ...
        while(!glfwWindowShouldClose(window))
        {
            ...
            if((currentFrame - lastFrame) > (1.0f / FPS))
            {
                ...
                earth->draw(projectionMatrix, viewMatrix);
